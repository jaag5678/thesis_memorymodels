...
\paragraph{3. Presence of cycles?}
    ...
    \footnotetext{$K_e$ and $K_d$ only apart from the new relation because these are the only valid cases where happens-before relations are preserved after reordering. So we need not consider cases such that $\reln{e}{hb}{k}$ or $\reln{k}{hb}{d}$ as the old relations they are covered by $K_e$ and $K_d$.}
