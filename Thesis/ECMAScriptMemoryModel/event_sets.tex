...
\subsection{Events}
    ...
    \subsubsection{Shared Memory (\set{SM}) Events} 
        
        This set is composed of two sets of events: 
        \begin{enumerate}
            \item Write events (\set{W}) which write to shared memory.
            \item Read events (\set{R}) which read from shared memory. 
        \end{enumerate}
        Events that belong to both Write and Read events are called Read-Modify-Write. 
    
    \subsubsection{Synchronize (\set{S}) Events} 
    ...
    \paragraph{Range ($\Re$)}
        ...
        \critic{red}{The range as per the ECMAScript standard denotes only the set of contiguous byte indices. The starting byte indexis kept separate. We find this to be unnecessary. Hence we define range to have starting index and size.}
        ...
        \critic{blue}{Note that two ranges being overlapping is different from them being equal. This distinction is used to definecertain things ahead in the model.}
    ...
    \paragraph{Value($V$)}  
       It is a function that maps a byte address given to the value that is stored in that address.For example, the byte address $k\text{ has the value } x_k$ will be depicted as:
        ...
        \critic{red}{We do not nee Value function as we do not use it anywhere nor is it specified in the standard}
...
%Tearing factor of events---------------------------------------------------------------------------------------------------------------------------
    ...
    \subsubsection{Tearing (Or not)}
        ...
        \critic{purple}{Perhaps place the blue text below as a footnote.}
